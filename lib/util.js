...
exports.checkParameters = (action) => {
    if (!hasRequiredParameters(action, ['token'])) {
        throw new Error('No deployment token was provided. You must provide the action with a Personal Access Token scoped to user:read.');
    }
...
exports.suppressSensitiveInformation = (str, action) => {
    let value = str;
    const orderedByLength = [action.token].filter(Boolean).sort((a, b) => b.length - a.length);
