...
function generateTemplate(response) {
    ...
    sponsorshipsAsMaintainer.nodes
        ...
        .map(({ sponsorEntity }) => {
        placeholder = placeholder += `<a href="https://github.com/${sponsorEntity.login}"><img src="https://github.com/${sponsorEntity.login}.png" /></a>
      }`;
    });
    return placeholder;
}
...
function generateFile(response) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            data = data.replace(/<!-- START COMMENT -->[\s\S]*?<!-- END COMMENT -->/g, generateTemplate(response));
